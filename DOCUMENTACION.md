...
│  │  ├─ board/                   # cancha, banca, tarjeta de jugador, export, compartir, zonas de influencia, flechas tácticas
...
### 5.13 Pizarra táctica avanzada: flechas (`/matches/[id]/board`) — Fase 7.1–7.2

Primera entrega de la pizarra táctica avanzada propuesta en una auditoría externa (`Propustas/Mejoras tácticas app de fútbol.md`). Alcance decidido con el usuario: motor completo de dibujo (flechas ahora, zonas libres con suavizado de curvas más adelante), sin automatizar RSVP por WhatsApp/SMS y sin offline-first completo (esas quedaron fuera de esta fase).

- **Modelo de datos**: nueva columna `match_lineups.graphics` (jsonb, `[]` por defecto — `0007_lineup_graphics.sql`). Se agregó como columna propia, **no anidada dentro de un jugador en `assignments`**, porque una flecha conecta a dos jugadores y no tiene un único dueño natural. Cada flecha guarda `{ id, type: "arrow", fromPlayerId, toPlayerId }` — sin coordenadas fijas, así que al mover a cualquiera de los dos jugadores la curva se recalcula sola en el siguiente render.
- **Modo "Dibujar"**: botón toggle (ícono de lápiz) en el header, junto al de zonas de influencia, visible solo para DT/Capitán. Con el modo activo:
  - Tocar un jugador lo marca como origen (aviso por *toast*); tocar un segundo jugador crea la flecha; volver a tocar el origen cancela la selección.
  - El arrastre de jugadores (dnd-kit) queda bloqueado mientras el modo está activo, para que no haya ambigüedad entre "quiero reposicionar" y "quiero dibujar una flecha" — es una desviación deliberada de la auditoría externa, que proponía resolver la convivencia con un sensor `data-no-dnd` sin modo explícito; un toggle Mover/Dibujar es más simple y sin ambigüedad, igual que en pizarras tácticas profesionales (TacticalPad, Coach's Eye).
  - Debajo de la cancha aparece la lista "Flechas (n)" (solo para DT/Capitán, solo con el modo activo) para quitar cualquiera con un botón ×.
- **Render**: `src/features/board/graphics-layer.tsx`, overlay SVG puro (mismo sistema de coordenadas en % que `influence-overlay.tsx`) que dibuja cada flecha como una curva cuadrática Bézier (`M ... Q ... `) con `<marker>` de cabeza de flecha (`orient="auto"`), recortada en ambos extremos para no taparse con el círculo del jugador. Color `var(--accent)` (turquesa de marca). Siempre visible para ambos roles — solo la edición está restringida a DT/Capitán.
- **Corrección técnica clave sobre la auditoría**: el documento asumía que las coordenadas de cancha viven en el espacio interno del `viewBox="0 0 100 150"` de `pitch-background.tsx` y proponía usar `SVGPoint`/`getScreenCTM().inverse()` para convertir toques de pantalla. En realidad las coordenadas de cada jugador son **porcentajes CSS** (`left: x%, top: y%`), desacopladas del viewBox interno de esa cancha — no hizo falta ninguna matriz de transformación, solo el mismo cálculo `(clientX - rect.left) / rect.width * 100` que ya usaba el drag & drop.
- **Sensor de dnd-kit "apagable"**: pasar `sensors={[]}` condicionalmente (el patrón que ya usaban `/board` y `/matches/[id]/board` desde la Fase 3 para distinguir DT de jugador) rompe un hook interno de dnd-kit (`useSensorSetup`) que usa la **cantidad** de sensores registrados como arreglo de dependencias de un `useEffect` — cambiar de 0 a 1 sensor entre renders dispara el error de React "changed size between renders". Se corrigió con un sensor custom (`ConditionalPointerSensor`, extiende `PointerSensor`) que **siempre** se registra (1 sensor, referencia estable vía `useMemo`) pero cuyo activador consulta un `ref` mutable (`!isDt || drawMode`) antes de delegar en la lógica original — la cantidad de sensores nunca cambia, solo si el activador deja pasar el gesto. Aplica también, de forma preventiva, a `/matches/[id]/board`'s uso previo del mismo patrón con Fase 3-6.
- **Pendiente de Fase 7**: 7.3 (zonas libres a mano alzada con simplificación Ramer-Douglas-Peucker + spline Catmull-Rom + conversión a Bézier) y 7.4 (vista "Mi plan de juego" que atenúa al resto del equipo y resalta los gráficos/instrucciones del jugador logueado).
- Probado de punta a punta contra Supabase real: DT crea una flecha Israel Torres → Hector Andahur (persistida, coordenadas verificadas en el DOM), la quita desde la lista, la vuelve a crear, y un jugador logueado la ve en modo solo lectura sin poder editarla ni ver el botón de modo dibujo.
...
- **Fase 7 — Pizarra táctica avanzada** 🔶 en progreso (7.1–7.2 completados) — evolución propuesta por una auditoría externa (`Propustas/Mejoras tácticas app de fútbol.md`). Flechas dinámicas entre jugadores con modo de dibujo dedicado ✅, ver sección 5.13. Pendiente: 7.3 (zonas libres a mano alzada con suavizado Ramer-Douglas-Peucker + Catmull-Rom + Bézier) y 7.4 (vista "Mi plan de juego" personalizada por jugador). La misma auditoría propuso además resiliencia offline (acordado: solo lectura cacheada, prioridad baja) y confirmación de asistencia por WhatsApp/SMS (descartada por costo/complejidad frente al flujo actual).
