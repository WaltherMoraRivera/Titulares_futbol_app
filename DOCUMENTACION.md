...
### 5.13 Pizarra táctica avanzada: flechas y zonas (`/matches/[id]/board`) — Fase 7.1–7.3
...
- **Pendiente de Fase 7**: 7.4 (vista "Mi plan de juego" que atenúa al resto del equipo y resalta los gráficos/instrucciones del jugador logueado).
- Probado de punta a punta contra Supabase real: DT crea una flecha Israel Torres → Hector Andahur (persistida, coordenadas verificadas en el DOM), la quita y la vuelve a crear; DT dibuja una zona a mano alzada (simulando el gesto con eventos de puntero, verificando visualmente la curva suave resultante), la gestiona desde la lista; un jugador logueado ve ambos gráficos en modo solo lectura, sin ver ninguno de los dos botones de herramienta.
...
- **Fase 7 — Pizarra táctica avanzada** 🔶 en progreso (7.1–7.3 completados) — evolución propuesta por una auditoría externa (`Propustas/Mejoras tácticas app de fútbol.md`). Flechas dinámicas entre jugadores y zonas libres a mano alzada con suavizado Ramer-Douglas-Peucker + Catmull-Rom + Bézier ✅, ver sección 5.13. Pendiente: 7.4 (vista "Mi plan de juego" personalizada por jugador). La misma auditoría propuso además resiliencia offline (acordado: solo lectura cacheada, prioridad baja) y confirmación de asistencia por WhatsApp/SMS (descartada por costo/complejidad frente al flujo actual).
