...
### 5.13 Pizarra táctica: formación general + mapas individuales por jugador — Fase 7 (reestructurada, ronda 2)

Pizarra táctica avanzada propuesta en una auditoría externa (`Propustas/Mejoras tácticas app de fútbol.md`). Implementada en tres rondas: una primera versión (7.1–7.4) con flechas/zonas libres dibujadas sobre la formación general más un modo "Mi plan de juego"; una segunda ronda que separó la formación general del contenido táctico individual en dos pantallas distintas (porque dibujar sobre la formación completa la volvía ilegible); y esta **tercera ronda**, a pedido explícito del usuario tras usar la anterior, que reemplaza las flechas automáticas por líneas de conexión neutras, agrega una herramienta de flechas libres con color, extiende las zonas con color, y cambia la selección de compañeros de una lista de chips a selección directa tocando la cancha. Esta sección documenta el diseño final; no hubo migración de datos entre rondas (mismos partidos de prueba, nunca datos reales).
...
interface PlayerTacticalMap {
  ...
  arrows: ArrowGraphic[];       // NUEVO: flechas libres, independientes de connectedPlayerIds
  zones: ZoneGraphic[];
}
```
Antes las flechas se derivaban automáticamente una por cada `connectedPlayerIds` (con color fijo). Ahora **`connectedPlayerIds` ya no genera flechas**: solo dibuja una línea blanca semi-transparente sin punta (`stroke="white" strokeOpacity={0.35}`, sin `<marker>`) entre el dueño y cada compañero curado, indicando la relación sin imponer una dirección. Las flechas pasaron a ser objetos independientes en `arrows[]`, dibujadas libremente por el DT entre cualquier par de jugadores visibles (dueño↔compañero o compañero↔compañero), cada una con su propio color. Render en `src/features/board/player-map-layer.tsx`: líneas de conexión → zonas → flechas, en ese orden; dos `<marker>` de cabeza de flecha en `<defs>` (uno por color, `TACTICAL_COLOR_HEX` en `src/utils/tactical-colors.ts`) porque el `fill` de un marker no se puede sobrescribir por instancia.

**Selección de compañeros — ahora tocando la cancha, no una lista de chips**: el DT entra en modo edición con el botón **"Editar compañeros"**; en ese modo, todos los jugadores del campo que no son el dueño se muestran atenuados (`opacity: 0.3`) directamente sobre la mini-cancha, y tocar a cualquiera lo agrega/quita del roster curado (`opacity: 1` si está seleccionado) llamando a `setConnectedPlayers(ownerId, ids)`. El botón **"Confirmar"** sale del modo edición: a partir de ahí la mini-cancha vuelve a mostrar únicamente al dueño y a los compañeros curados (ya no se ven los no seleccionados), dejando la pantalla limpia para trabajar con flechas y zonas. Reemplaza por completo la lista de chips de la ronda anterior.

**Herramienta de flechas libres** (ícono lápiz, solo DT/Capitán, deshabilitada durante "Editar compañeros"): al activarla aparece el selector de color (dos pastillas, verde/rojo, estado compartido `drawColor` con la herramienta de zonas) y las tarjetas de los jugadores visibles pasan a ser botones tocables; el primer toque fija el origen, el segundo el destino, y `addArrow(ownerId, fromId, toId, color)` persiste la flecha. Un banner instructivo indica el paso actual ("Elegí el jugador de origen" / "Elegí el destino"). Lista "Flechas (n)" debajo de la cancha con un punto de color, texto `Origen → Destino`, y botón de eliminar por fila (`removeArrow`).
...
**Mini-cancha**: la lógica de render que antes vivía en `src/features/board/player-map-field.tsx` (eliminado en esta ronda) se integró directamente en la página (`src/app/matches/[id]/board/[playerId]/page.tsx`), porque el modo "Editar compañeros" necesita alternar entre dos conjuntos de jugadores mostrados (todos atenuados vs. solo los curados) con más ramificación de la que convenía mantener detrás de un límite de props limpio. Las tarjetas siguen siendo estáticas posicionadas por porcentaje (no `useDraggable`); la del dueño y las de los compañeros/candidatos ahora son `<button>` (antes `<div>` para el dueño) para ser tocables desde las herramientas de selección y flechas.
...
- Probado de punta a punta contra Supabase real: desde la formación general, el DT navega al mapa de un jugador ("Israel"); entra en "Editar compañeros", selecciona 2 (quedan atenuados los no elegidos), confirma (la cancha pasa a mostrar solo 3 tarjetas); activa la herramienta de flechas, elige rojo, toca a Israel como origen y a un compañero como destino — la flecha se persiste y aparece en la lista "Flechas (1)" con el punto rojo y el texto `Israel Torres → Hector Andahur`, verificado también a nivel DOM (`path[marker-end="url(#arrow-head-red)"]` con `stroke="#ef4444"`); dibuja una zona verde y la deshace (queda solo el cambio de flecha, verificado); vuelve a "Editar compañeros" y confirma que los 2 compañeros previos siguen marcados como seleccionados. Un bug encontrado y corregido durante esta prueba: la tarjeta del dueño no era tocable (era un `<div>`), impidiendo usarlo como origen de una flecha — corregido cambiándola a `<button>`.
...
- **Fase 7 — Pizarra táctica** ✅ completado — evolución propuesta por una auditoría externa (`Propustas/Mejoras tácticas app de fútbol.md`), reestructurada después en dos rondas a pedido del usuario: primero en dos pantallas separadas (`/matches/[id]/board` formación general sin dibujo, `/matches/[id]/board/[playerId]` mapa táctico individual), y luego —tras usarla— reemplazando las flechas automáticas por líneas de conexión neutras, agregando una herramienta de flechas libres con color (verde/rojo), extendiendo las zonas con el mismo color, y cambiando la selección de compañeros de una lista de chips a selección directa tocando la cancha (con botones Confirmar/Editar compañeros), todo con suavizado Ramer-Douglas-Peucker + Catmull-Rom + Bézier para las zonas y deshacer con historial separado por jugador, ver sección 5.13. La misma auditoría propuso además resiliencia offline (acordado: solo lectura cacheada, prioridad baja, todavía no implementada) y confirmación de asistencia por WhatsApp/SMS (descartada por costo/complejidad frente al flujo actual).
