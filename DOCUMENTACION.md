...
│  │  ├─ board/                   # cancha, banca, tarjeta de jugador, export, compartir, zonas de influencia, flechas y zonas tácticas
...
- **Si ya existe formación:** ambos ven la misma cancha + banca (`Field`, `BenchStrip`, reutilizados de `/board`), pero con comportamiento distinto según el rol:
  ...
  - **Jugador**: el arrastre está deshabilitado; puede tocar a cualquier jugador para ver su info y, si tiene, sus instrucciones tácticas en texto de solo lectura — sin botón para sacarlo de la cancha.
...
### 5.13 Pizarra táctica avanzada: flechas, zonas, deshacer y "Mi plan de juego" (`/matches/[id]/board`) — Fase 7 completa

Pizarra táctica avanzada propuesta en una auditoría externa (`Propustas/Mejoras tácticas app de fútbol.md`). Alcance decidido con el usuario: motor completo de dibujo (flechas y zonas libres con suavizado de curvas), sin automatizar RSVP por WhatsApp/SMS y sin offline-first completo (esas quedaron fuera de esta fase).

- **Modelo de datos**: nueva columna `match_lineups.graphics` (jsonb, `[]` por defecto — `0007_lineup_graphics.sql`). Se agregó como columna propia, **no anidada dentro de un jugador en `assignments`**, porque un gráfico como una flecha conecta a dos jugadores y no tiene un único dueño natural. Cada flecha guarda `{ id, type: "arrow", fromPlayerId, toPlayerId }` (sin coordenadas fijas — se recalcula sola si se mueve a cualquiera de los dos jugadores); cada zona guarda `{ id, type: "zone", points }` (coordenadas propias, independiente de los jugadores).
- **Tres herramientas** en el header, dos toggles mutuamente excluyentes (`tool: "move" | "arrow" | "zone"`) visibles solo para DT/Capitán, junto al de zonas de influencia (Fase 6):
  - **Flechas** (ícono de lápiz): tocar un jugador lo marca como origen (aviso por *toast*), tocar un segundo jugador crea la flecha, volver a tocar el origen cancela.
  - **Zonas** (ícono de formas): dibujar el contorno con el dedo sobre la cancha y soltar para cerrarlo — el trazo crudo se simplifica (Ramer-Douglas-Peucker) y se suaviza a una curva cerrada (Catmull-Rom → Bézier) antes de guardarse.
  - El arrastre de jugadores (dnd-kit) queda bloqueado mientras cualquiera de las dos herramientas está activa, para que no haya ambigüedad entre "quiero reposicionar" y "quiero dibujar" — desviación deliberada de la auditoría externa, que proponía resolver la convivencia con un sensor `data-no-dnd` sin modo explícito; un toggle Mover/Dibujar es más simple y sin ambigüedad, igual que en pizarras tácticas profesionales (TacticalPad, Coach's Eye).
  - Debajo de la cancha aparecen las listas "Flechas (n)" y "Zonas (n)" (solo para DT/Capitán, solo con alguna herramienta activa) para quitar cualquier elemento con un botón ×.
- **Deshacer** (ícono ↩, solo DT/Capitán): pila de deshacer acotada a 20 pasos (`graphicsHistory` en `useMatchLineupStore`) que guarda el estado anterior de `graphics` antes de cada `addArrow`/`addZone`/`removeGraphic`. El botón restaura el paso inmediatamente anterior (persistido en Supabase) y queda deshabilitado cuando no hay nada que deshacer; la pila se reinicia al cargar un partido nuevo o al armar una formación desde cero. Es deshacer específico de flechas/zonas, no de toda la alineación (mover un jugador o editar instrucciones no entra en esta pila).
- **Render**: `src/features/board/graphics-layer.tsx`, overlay SVG puro (mismo sistema de coordenadas en % que `influence-overlay.tsx`) que dibuja las flechas como curvas cuadráticas Bézier (`M ... Q ...`) con `<marker>` de cabeza de flecha (`orient="auto"`, color `var(--accent)`, turquesa de marca) y las zonas como contornos cerrados (`var(--primary)`, dorado-bronce, relleno translúcido). Siempre visibles para ambos roles — solo la edición está restringida a DT/Capitán.
- **Captura del trazo libre**: `src/features/board/zone-drawing-layer.tsx`, una superficie `touch-action: none` que solo se monta cuando la herramienta de zonas está activa (`pointerdown`/`pointermove`/`pointerup` acumulan puntos en estado local, con vista previa en vivo) y `src/utils/curve-smoothing.ts`, las utilidades matemáticas puras: `simplifyPoints` (Ramer-Douglas-Peucker recursivo) y `catmullRomToBezierPath` / `catmullRomToClosedBezierPath` (conversión de tangentes de Catmull-Rom uniforme a puntos de control de Bézier cúbico, ya que SVG no soporta Catmull-Rom nativo en su atributo `d`).
- **Correcciones técnicas clave sobre la auditoría**:
  - El documento asumía que las coordenadas de cancha viven en el espacio interno del `viewBox="0 0 100 150"` de `pitch-background.tsx` y proponía usar `SVGPoint`/`getScreenCTM().inverse()` para convertir toques de pantalla. En realidad las coordenadas de cada jugador (y ahora también las de los trazos) son **porcentajes CSS** (`left: x%, top: y%`), desacopladas del viewBox interno de esa cancha — no hizo falta ninguna matriz de transformación, solo el mismo cálculo `(clientX - rect.left) / rect.width * 100` que ya usaba el drag & drop.
  - El documento sugería Catmull-Rom **centrípeto** (parametrización por raíz de la distancia, más robusta contra cúspides en trazos muy irregulares); se implementó la variante **uniforme** (más simple) por ser suficiente para trazos tácticos dibujados a mano, no para ilustración vectorial de precisión — si en el futuro se ven curvas con auto-intersecciones raras en trazos muy angulosos, ese es el punto a revisar.
- **Sensor de dnd-kit "apagable"**: pasar `sensors={[]}` condicionalmente (el patrón que ya usaban `/board` y `/matches/[id]/board` desde la Fase 3 para distinguir DT de jugador) rompe un hook interno de dnd-kit (`useSensorSetup`) que usa la **cantidad** de sensores registrados como arreglo de dependencias de un `useEffect` — cambiar de 0 a 1 sensor entre renders dispara el error de React "changed size between renders". Se corrigió con un sensor custom (`ConditionalPointerSensor`, extiende `PointerSensor`) que **siempre** se registra (1 sensor, referencia estable vía `useMemo`) pero cuyo activador consulta un `ref` mutable (`!isDt || tool !== "move"`) antes de delegar en la lógica original — la cantidad de sensores nunca cambia, solo si el activador deja pasar el gesto.
- **"Mi plan de juego"** (ícono de mira, Fase 7.4): botón visible para cualquier cuenta con un `playerId` reclamado (jugador, o admin si reclama uno). Al activarlo:
  - Las tarjetas de los demás jugadores (cancha y banca) bajan su opacidad a `0.25`; la del jugador enfocado queda a opacidad completa con un `drop-shadow` en el turquesa de marca.
  - Las flechas que no involucran al jugador enfocado (ni como origen ni como destino) bajan su `stroke-opacity` a `0.15`; las que sí lo involucran se resaltan con el mismo `drop-shadow`.
  - **Las zonas no se atenúan**: al no estar ancladas a un jugador específico en el modelo de datos actual (son formas independientes, no "propiedad" de nadie), se tratan como contexto táctico general relevante para todo el equipo, no como algo individual a ocultar.
  - Si el jugador enfocado tiene instrucciones tácticas cargadas, aparecen en un panel fijo "Tus instrucciones" debajo de la cancha, sin necesidad de tocar su propia tarjeta.
- Probado de punta a punta contra Supabase real: DT crea dos flechas y prueba Deshacer dos veces en cadena (quita la más reciente primero, después la anterior, botón queda deshabilitado sin historial); DT dibuja una zona a mano alzada (curva suave verificada visualmente); un jugador real (sesión propia, no admin) reclama su número, activa "Mi plan de juego" y ve el panel de instrucciones y el resalte (`filter: drop-shadow`) aplicado únicamente a su propia tarjeta y a la flecha que lo involucra, confirmado a nivel de DOM.
...
- `0007_lineup_graphics.sql` — pizarra táctica avanzada (Fase 7): columna `match_lineups.graphics` (jsonb, `[]` por defecto) para flechas y, más adelante, zonas libres. Sin cambios de RLS — cubierta por las policies ya existentes de `match_lineups`. Ver sección 5.13.
...
- **Fase 7 — Pizarra táctica avanzada** ✅ completado — evolución propuesta por una auditoría externa (`Propustas/Mejoras tácticas app de fútbol.md`). Flechas dinámicas entre jugadores, zonas libres a mano alzada con suavizado Ramer-Douglas-Peucker + Catmull-Rom + Bézier, deshacer acotado a 20 pasos, y vista "Mi plan de juego" que atenúa al resto del equipo y resalta los gráficos/instrucciones del jugador logueado, ver sección 5.13. La misma auditoría propuso además resiliencia offline (acordado: solo lectura cacheada, prioridad baja, todavía no implementada) y confirmación de asistencia por WhatsApp/SMS (descartada por costo/complejidad frente al flujo actual).
