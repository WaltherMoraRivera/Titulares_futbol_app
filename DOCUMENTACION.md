...
### 5.13 Pizarra táctica: formación general + mapas individuales por jugador — Fase 7 (reestructurada)

Pizarra táctica avanzada propuesta en una auditoría externa (`Propustas/Mejoras tácticas app de fútbol.md`). Implementada en dos rondas: una primera versión (7.1–7.4) con flechas/zonas libres dibujadas sobre la formación general más un modo "Mi plan de juego" que atenuaba al resto del equipo, y una **segunda reestructuración** — a pedido explícito del usuario tras usarla — que separó la formación general del contenido táctico individual en dos pantallas distintas, porque dibujar varias flechas/zonas sobre la formación completa la volvía ilegible. Esta sección documenta el diseño final; la primera versión quedó completamente reemplazada (sin retrocompatibilidad de datos — no había partidos reales usándola).

**Dos pantallas separadas:**

- **`/matches/[id]/board` — formación general.** Solo posiciones (drag & drop), banca, zonas de influencia (Fase 6) y compartir. Sin ninguna herramienta de dibujo. Tocar a un jugador abre una ficha con sus datos básicos y, si corresponde (ver más abajo), un botón **"Ver mapa táctico"** que lleva a su pantalla individual.
- **`/matches/[id]/board/[playerId]` — mapa táctico individual (nueva ruta).** Muestra únicamente a ese jugador y a los compañeros que el DT curó para él — no a los 11 titulares — con flechas de conexión automáticas hacia cada uno, zonas propias dibujadas a mano alzada, e instrucciones tácticas en texto. Es la pantalla dedicada a "qué necesita saber este jugador", separada a propósito de "dónde para el equipo".

**Quién ve el enlace "Ver mapa táctico"** desde la formación general: el DT/Capitán para cualquier jugador en cancha, o un jugador logueado únicamente para su propia tarjeta — nunca para la de un compañero (es contenido personalizado, no algo para curiosear). Solo aparece si el jugador está en cancha (`assignments`), porque el mapa necesita su posición para ubicarlo.

**Modelo de datos**: columna `match_lineups.tactical_maps` (jsonb — renombrada desde `graphics` en `0008_player_tactical_maps.sql`, mismo dato subyacente, sin migración de esquema real). Cada entrada es un `PlayerTacticalMap`:
```ts
interface PlayerTacticalMap {
  ownerId: string;              // el jugador dueño de este mapa
  connectedPlayerIds: string[]; // compañeros curados por el DT
  zones: ZoneGraphic[];         // zonas propias de este mapa (a mano alzada)
}
```
Ya no existen flechas como objetos independientes: se derivan en el render, una por cada `connectedPlayerIds`, desde la posición del dueño hacia la de cada compañero (`src/features/board/player-map-layer.tsx`, reutiliza la misma curva cuadrática Bézier + `<marker>` de cabeza de flecha de la versión anterior). Las zonas también pasaron a vivir **dentro** del mapa de cada jugador en vez de ser generales del equipo — decisión tomada con el usuario para que todo el contenido táctico esté organizado por jugador de forma consistente.

**Selección de compañeros**: en la página individual, el DT ve una lista de chips con todos los demás jugadores en cancha (no la banca — un compañero necesita coordenada de formación para poder dibujarle una flecha) y toca para agregar/quitar del mapa. Cada toque llama a `setConnectedPlayers(ownerId, ids)`, que persiste el arreglo completo — deshacer un error de selección es tan simple como volver a tocar el chip.

**Dibujo de zonas**: mismo mecanismo que la versión anterior (`src/features/board/zone-drawing-layer.tsx`, `src/utils/curve-smoothing.ts` con Ramer-Douglas-Peucker + Catmull-Rom → Bézier, sin cambios), ahora activado desde la página individual y guardado dentro del mapa del jugador que se está viendo.

**Deshacer** (ícono ↩, solo DT/Capitán): a diferencia de la versión anterior (una sola pila global), ahora hay **una pila de deshacer separada por jugador** (`mapHistory: Record<ownerId, PlayerTacticalMap[]>` en `useMatchLineupStore`) — deshacer en el mapa de un jugador nunca puede afectar por accidente el de otro, algo que sí podía pasar antes si el DT alternaba entre editar distintos jugadores. Acotada a 20 pasos por jugador; se reinicia al cargar un partido nuevo o armar una formación desde cero.

**Mini-cancha sin drag & drop**: `src/features/board/player-map-field.tsx` reutiliza `PitchBackground`/`PlayerCardVisual`/`PlayerMapLayer`, pero las tarjetas del dueño y sus compañeros son `<div>` estáticos posicionados por porcentaje (no `PlayerCard` con `useDraggable`) — las posiciones vienen fijas de la formación general, no se editan desde acá.

**Correcciones técnicas de la auditoría externa que se mantienen** (documentadas ya en la versión anterior, siguen aplicando): las coordenadas de cancha son porcentajes CSS, no requieren `SVGPoint`/`getScreenCTM()`; Catmull-Rom uniforme en vez de centrípeto; el sensor "apagable" de dnd-kit (`ConditionalPointerSensor`, siempre registrado con activador condicionado por `ref`) sigue vigente en la formación general para no romper el hook interno `useSensorSetup` de dnd-kit al alternar DT/jugador.

- Probado de punta a punta contra Supabase real: desde la formación general, el DT toca a un jugador y navega a su mapa; selecciona 2 compañeros (la mini-cancha pasa de mostrar 11 tarjetas a mostrar solo 3, con 2 flechas); dibuja una zona y la deshace (queda solo el cambio de compañeros, verificado); cierra sesión y entra con una cuenta de jugador real que reclama a ese mismo jugador — ve exactamente su mapa curado (3 tarjetas, flechas, instrucciones) en modo solo lectura, sin ningún control de edición; al tocar la tarjeta de un compañero en la formación general no ve el enlace al mapa (privado), pero sí al tocar la suya propia.
...
- **Fase 7 — Pizarra táctica** ✅ completado — evolución propuesta por una auditoría externa (`Propustas/Mejoras tácticas app de fútbol.md`), reestructurada después a pedido del usuario en dos pantallas separadas: `/matches/[id]/board` (formación general del equipo, sin dibujo) y `/matches/[id]/board/[playerId]` (mapa táctico individual por jugador — compañeros curados por el DT con flechas automáticas, zonas propias a mano alzada con suavizado Ramer-Douglas-Peucker + Catmull-Rom + Bézier, y deshacer con historial separado por jugador), ver sección 5.13. La misma auditoría propuso además resiliencia offline (acordado: solo lectura cacheada, prioridad baja, todavía no implementada) y confirmación de asistencia por WhatsApp/SMS (descartada por costo/complejidad frente al flujo actual).
